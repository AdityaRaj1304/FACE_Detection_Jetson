...
export default function Settings() {
  ...
  const handleSimulateEvent = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await axios.post('http://localhost:3000/api/v1/events/attendance', payload);
      
      if (res.data?.success) {
        setSimMsg({ text: `Simulated event sent successfully for ${payload.student_name}!`, error: false });
      }
    } catch (err) {
...
with open('/Users/adityakumarsingh/Documents/Face_Detection_Jetson/warden-dashboard/src/pages/Settings.tsx', 'w') as f:
    f.write(content)
print("Settings.tsx rewritten with Simulator")
